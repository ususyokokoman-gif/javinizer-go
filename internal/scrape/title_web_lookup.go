...
func (s *Scraper) lookupCatalogIDOnWeb(ctx context.Context, title string) (string, error) {
	...
	var merged []titleWebSearchResult
	var lastErr error
	for _, q := range queries {
		...
		if id, ok := chooseCatalogCandidate(title, merged); ok && candidateHasTrustedEvidence(id, merged) { return id, nil }
	}
	if id, ok := chooseCatalogCandidate(title, merged); ok { return id, nil }
	if lastErr != nil && len(merged) == 0 { return "", lastErr }
...
func (s *Scraper) fetchTitleWebSearch(ctx context.Context, provider, query string) ([]titleWebSearchResult, error) {
	if provider != "google" { return nil, fmt.Errorf("unsupported web search provider %q; Google is the only provider", provider) }
	...
	if s.cfg != nil && strings.TrimSpace(s.cfg.UserAgent) != "" { ua = s.cfg.UserAgent }
	req.Header.Set("User-Agent", ua)
	req.Header.Set("Accept", "text/html,application/xhtml+xml")
	req.Header.Set("Accept-Language", "ja-JP,ja;q=0.9,en-US;q=0.7,en;q=0.5")
	resp, err := s.httpClient.Do(req)
	if err != nil { return nil, fmt.Errorf("Google search request failed: %w", err) }
	if resp == nil { return nil, fmt.Errorf("Google search returned nil response") }
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 { return nil, fmt.Errorf("Google search returned HTTP %d", resp.StatusCode) }
	doc, err := goquery.NewDocumentFromReader(io.LimitReader(resp.Body, maxWebSearchBody))
	if err != nil { return nil, fmt.Errorf("parse Google search page: %w", err) }
	results := parseTitleWebResults("google", doc)
	if isGoogleSearchInterstitial(doc, results) { return fetchGoogleSearchWithHeadlessBrowser(ctx, endpoint) }
	return results, nil
}
